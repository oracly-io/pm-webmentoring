...
export function getMentoringStat(state) {
  const stat = get(state, ['statistics', 'stat'])

  return stat
}

export function getMentoringFundERC20(state, erc20) {
  const fund = get(state, ['statistics', 'funds', erc20])

  return fund
}

export function getStatisticsBlockNumber(state) {
  const blockNumber = get(state, ['statistics', 'blockNumber'])
